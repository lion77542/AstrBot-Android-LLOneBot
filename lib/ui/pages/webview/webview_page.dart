...
class _WebViewPageState extends State<WebViewPage> {
  ...
  late final WebViewController _llBotController;
  ...
  void _initAstrBotController() {
    ...
    // 刷新保护：首次加载失败时自动重试（修复 #10 白屏）
    _astrBotController.setNavigationDelegate(
      NavigationDelegate(
  ...
  void _initLLBotController() {
    _llBotController = WebViewController()
      ..setJavaScriptMode(JavaScriptMode.unrestricted)
      ..setBackgroundColor(Colors.white)
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageFinished: (String url) {
            _disableZoom(_llBotController);
            _injectPasswordScript(_llBotController, url);
          },
          onWebResourceError: (WebResourceError error) {
            debugPrint('LLBot WebView error: ${error.description}');
          },
        ),
      );
    ...
    if (_llBotController.platform is AndroidWebViewController) {
      AndroidWebViewController.enableDebugging(true);
      final androidController = _llBotController.platform as AndroidWebViewController;
      androidController
          .setMediaPlaybackRequiresUserGesture(false);
      // 设置混合内容模式以提高兼容性（Android 9+ 需要）
    ...
    _llBotController.addJavaScriptChannel(
      'Android',
      onMessageReceived: (JavaScriptMessage message) {
        ...
        child: Obx(() {
          ...
          if (_previousNavItemCount != 0 && _previousNavItemCount != currentNavItemCount) {
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) setState(() => _currentIndex = settingsIndex);
            });
