...
class HomeController extends GetxController {
  ...
  final RxString llbotWebUiToken = ''.obs; // 存储 LLBot WebUI Token
  final RxBool _isQrcodeShowing = false.obs;
  final RxBool llbotWebUiEnabledRx = false.obs; // GetX 响应式变量用于导航栏更新
  ...
  void initQrcodeListener() {
    if (llbotTerminal == null) return;

    _qrcodeSubscription = llbotTerminal!.output
        .cast<List<int>>()
        .transform(const Utf8Decoder(allowMalformed: true))
        .listen((event) async {
      // 先判断订阅是否已取消，避免重复处理
      ...
      // 检测指令1显示二维码
      if ((event.contains('二维码已保存到') || event.contains('qrcode')) && !_isQrcodeShowing.value) {
        _isQrcodeShowing.value = true;
        // LLBot 二维码显示在 WebUI 上（http://127.0.0.1:3080），不在本地文件系统找
        // 改为通过 WebUI 处理登录
  ...
  void syncProgress() {
    ...
    progressDesFile.watch(events: FileSystemEvent.all).listen((event) async {
      if (event.type == FileSystemEvent.modify) {
        ...
        // 当进度到达 "LLBot 已安装" 时，启动 LLBot 终端
        if (content.contains('LLBot ${S.current.installed}')) {
          llbotTerminal?.writeString('$command\n');
          bumpProgress();
          Log.i('检测到 LLBot 已安装，启动 LLBot 终端', tag: 'AstrBot');
        }
  ...
  Future<void> loadAstrBot() async {
    ...
    // 防卡死保护：5分钟后强制跳转 WebView（即使进度检测丢失）
    Future.delayed(const Duration(minutes: 5), () {
      if (!webviewHasOpen && _isAppInForeground) {
        Log.w('防卡死触发：5分钟超时，强制跳转 WebView', tag: 'AstrBot');
        _isLocalhostDetected = true;
        _isQrcodeProcessed = true;
        _checkAndNavigateToWebview();
      }
  ...
  void onInit() {
    ...
    // 初始化 LLBot WebUI 启用状态
    llbotWebUiEnabledRx.value = llbotWebUiEnabled.get() ?? false;
  ...
  // 更新 LLBot WebUI 启用状态（用于同步响应式变量）
  void setLLBotWebUiEnabled(bool value) {
    llbotWebUiEnabled.set(value);
    llbotWebUiEnabledRx.value = value;
  }
  ...
  void onClose() {
    ...
    try {
      ...
      if (llbotTerminal != null) {
        Log.i('正在关闭 LLBot 终端进程...', tag: 'AstrBot-LLBot');
        llbotTerminal?.kill();
        llbotTerminal = null;
      }
    } catch (e) {
