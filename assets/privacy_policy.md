...
- 本应用（AstrBot-Android-LLOneBot）是完全免费的开源项目，**从未授权任何个人或平台进行收费分发**
...
本政策适用于您在使用 _AstrBot-Android-LLOneBot_ 移动应用过程中的个人信息处理活动（包括后续版本、功能模块更新或扩展）。本应用基于AstrBot聊天机器人框架、LLBot(幸运莉莉娅)消息适配器与Code LFA的Ubuntu容器环境打造，您在使用第三方框架及适配器时，还应遵守其相应的隐私政策。
...
- 我们**不要求**您注册账户。
- 您通过 LLBot 适配器登录的 QQ 账号信息由该适配器在本地处理，我们不直接收集和存储您的 QQ 账号凭证，相关数据仅保存在您的设备本地。
...
- **前台服务权限**（FOREGROUND_SERVICE, FOREGROUND_SERVICE_DATA_SYNC）：用于在后台保持 AstrBot 和 LLBot 服务运行（必要）。
...
- **LLBot 消息适配器**：详情请参考其官方隐私说明（https://luckylillia.com/）
...
- **WebView 访问**：访问本地服务 (127.0.0.1)，用于 AstrBot 和 LLBot 管理界面
